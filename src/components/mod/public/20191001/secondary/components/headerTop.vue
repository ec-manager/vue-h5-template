        ...
        <header class="head" v-if="header.is_show">
            ...
            <div class="time-line-box">
                <swiper class="time-line-nav"
                    ...
                    v-if="timeLine.is_show">
                    <swiper-slide v-for="(item, index) of timeLine.list"
                        :key="index"
            ...
            <!-- 活动规则 -->
            <div class="rule-btn" data-fql-stat="BTN_RULE" @click="togglePopRuleStatus()">活动规则</div>
        </header>
        <!-- header]] -->
...
/**
 * @author melody
 * @des 6周年顶部banner组件 含logo 时间线 活动规则按钮 banner
 * @params options：楼层配置
 */
...
export default {
    ...
    methods: {
        // 高亮当前时间轴
        setTimeLine () {
            this.header.time_line.list.forEach((item, index) => {
