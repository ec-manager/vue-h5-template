    ...
    <section class="category-coupon-box">
        ...
        <div v-for="(timeItem, timeKey) in options.time_list"
            ...
            <template v-if="!options.is_template">
                ...
                <!-- 电商新优惠券，需要配置中配置 new_shoper_coupon_list 字段 -->
                    <template v-if="isShopNewUser">
                        <div v-for="(item,index) in timeItem.new_shoper_coupon_list"
                            :key="index + 'newer'"
                            class="coupon-list"
                            :class="'coupon-list-'+item.cols">
...
/**
 * @author melody
 * @des 6周年类目券组件，当前未设置券的高度，存在高度未占位的问题，故设置minHeight
 * @params options：楼层配置
 * @params isLogin：是否登录
