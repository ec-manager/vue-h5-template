<template>
    <div>
        <a class="coupon-item"
            ...
            <img class="covimg" v-lazy="authImg"/>
            <div class="btn-text">立即<br>开通</div>
        </a>
        ...
        <template v-else-if="!isNewer && isAuth">
            <a v-if="isShowOlder" class="coupon-item" :style="{height: tool.toRem(coupHeight)}"
                :key="2"
                v-href="olderLink"
                v-stat="'BTN_LINK_OLDER'">
                <img class="covimg" v-lazy="olderImg"/>
            </a>
        </template>
    </div>
</template>
