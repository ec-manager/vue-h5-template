/**
*@name app bridge
*@author denyszhou <author@lexin.com>
*@version 2.1
*@description 封装所有可用的js api方法
*@modify 版本改动点，升级注意事项
*-add 新增 淘宝登录授权api接口
*-modify ios修复appbridge注入时机调用方法，确保ios上调用方法时bridge已经被注入 2.0版本有概率出现appbridge注入不成功的现象
*/
...
const jsBridge = {
    ...
    // 设置返回调用
    setReturnFunc: function (callbackName, options = {}) {
    ...
    // 设置函数调用
    setCallFunc: function (method, options) {
        // 获取传给接口的回调函数名
    ...
    /**
     * @function getAliAuthLogin
     * @description [打开淘宝APP授权]
     * @param {Object} options 选项[必选]
     * @param {Function} options.onReturn [回调方法]
     */
    getAliAuthLogin: function (options) {
        // // 版本检查，不可用版本调用时报错
        if (AppBridge.isVersionBelow('5.14.0')) {
            let callVersion = this.getVersion();
            throw new Error('call getAliAuthLogin on an invalid app version[' + callVersion + ']');
        }
        ...
        // 判断是否为有效的回调
        if (Tool.isFunction(options.onReturn)) {
            this.setReturnFunc(callbackName, options);
        }
        let params = {
            callBackName: callbackName
        };
        return this.invoke('getAliAuthLogin', params);
    },

    /**
     * @function doAlibcTradeOpenUrl
     * @description [打开淘宝APP页面]
     * @param {Object} options 选项[必选]
     * @param {String} options.url 页面url [必填]
     * @param {String} options.identity 固定为 trade [必填]
     * @param {Object} options.taoKeParams 淘客参数，直接透传给百川SDK
     * @param {Object} options.trackParam 链路跟踪参数,直接透传给百川SDK
     * @param {Function} options.onReturn [回调方法]
     */
    doAlibcTradeOpenUrl: function (options = {}) {
        // // 版本检查，不可用版本调用时报错
        if (AppBridge.isVersionBelow('5.14.0')) {
            let callVersion = this.getVersion();
            throw new Error('call getAliAuthLogin on an invalid app version[' + callVersion + ']');
        ...
        let {url = '', identity = 'trade', taoKeParams = {}, trackParam = {}} = options;
        // 重新赋值需要的参数
        let params = {
            callBackName: callbackName,
            url,
        ...
        return this.invoke('doAlibcTradeOpenUrl', params);
    },

    /**
     * @function openJDMobile
     * @description [打开京东APP]
     * @param {Object} options 选项[必选]
     * @param {String} options.url 页面url [必填]
     * @param {Object} options.userInfo 附加参数
     * @param {Function} options.onReturn [回调方法]
     */
    openJDMobile: function (options = {}) {
        // // 版本检查，不可用版本调用时报错
        if (AppBridge.isVersionBelow('5.14.0')) {
            let callVersion = this.getVersion();
            throw new Error('call openJDMobile on an invalid app version[' + callVersion + ']');
        }
        let callbackName = this.getCallbackName('onReturn');
        ...
        return this.invoke('openJDMobile', params);
    },

    /**
     * @function isAppInstalled
     * @description [判断某个app是否已安装]
     * @param {Object} options 选项[必选]
     * @param {String} options.schemeName 判断是否安装对应的iOS APP
     * @param {Number} options.packageName 判断是否已经安装的安卓App包名
     * @param {String} options.onReturn [回调方法]
     *
     * setAsyncCall 调用返回值有误，暂不使用 可能与 invokeJSAPI调用有关，问题待定位
     */
    isAppInstalled: function (options) {
        let callbackName = this.getCallbackName('onReturn');
        let {schemeName = '', packageName = ''} = options;
        ...
        let ret = this.invoke('isAppInstalled', params);
        return ret;
    },
    /**
     * @function getStateBarHeight
     * @description [获取安全区高度]
     * @param {Object} options 选项[必选]
     * @param {String} options.onReturn [回调方法]
     */
    getStateBarHeight: function (options = {}) {
        // 获取传给接口的回调函数名
        let callbackName = this.getCallbackName('onReturn', 'getStateBarHeight');
        if (Tool.isFunction(options.onReturn)) {
            this.setCallback(callbackName, options.onReturn, function (res) {
                if (typeof res !== 'object') { // android返回返回字符串
                    res = JSON.parse(res || '');
        ...
        return this.invoke('getStateBarHeight', params);
    },
    /**
     * [获取用户信息]
     * @method getUserReportInfo
     * @param {object} options
     * @returns {object}
     * @example
     * @wiki http://wiki.fenqile.com/pages/viewpage.action?pageId=53281790
     AppBridge.invokeJsBridge('getUserReportInfo', {
                onReturn: function(data){}
            });
     */
    getUserReportInfo: function (options) {
        // 获取传给接口的回调函数名
        let callbackName = this.getCallbackName('onReturn', 'getUserReportInfo');
        if (Tool.isFunction(options.onReturn)) {
            this.setCallback(callbackName, options.onReturn, function (res) {
                if (typeof res !== 'object') { // android返回返回字符串
                    res = JSON.parse(res || '');
        ...
        return this.invoke('getUserReportInfo', params);
    }
};
